...
- The committed SPDX 2.3 NuGet snapshot covers nine unique packages (three direct runtime, five transitive
  runtime, one build-only), records content hashes/license evidence, and is regenerated byte-for-byte by the
  offline verifier. Dated NuGet advisory queries reported zero known vulnerabilities; D08 remains unproven
  until binary/network and deployed-site tracker scans run.
...
| NuGet dependency graph is inventoried and drift-checked | `scripts/New-DependencySbom.ps1`, `docs/Dependency-SBOM.spdx.json` | verifier dependency SBOM smoke; dated NuGet advisory queries |
