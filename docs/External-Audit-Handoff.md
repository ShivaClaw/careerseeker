...
| Trusted-tester ZIP carries source provenance, payload checksums, and provider-key quickstart guidance | `scripts/Package-AlphaRelease.ps1`, `scripts/Test-AlphaReleasePackage.ps1` | release manifest source commit checks; audit snapshot provenance checks; README-alpha provider-key checks; SHA-256 checksum smoke |
