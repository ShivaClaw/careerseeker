...
- `scripts/Package-AlphaRelease.ps1` creates a trusted-tester ZIP with the published executable, native runtime
  dependencies, double-click setup/profile/provider/Gmail/demo/scout/live/evidence/verify/dashboard launchers, workspace initializer, dashboard/helper self-check scripts, quickstart, audit snapshot, release manifest,
...
- The alpha executable can safely import a local alpha ZIP package into `.appdata/imported` by default, reject
  unsafe ZIP paths, preserve existing files unless `--overwrite` is passed, and verify the restored audit chain.
