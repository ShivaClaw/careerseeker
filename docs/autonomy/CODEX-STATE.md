...
| R6 | IN PROGRESS | R6(a) is complete on `codex/r6-delete-all-data`: the installed executable now has a separately confirmed, exact-path `delete-all-data` mode; six new EngineHarness assertions cover path binding, mismatch/root refusal, complete removal, and already-absent reporting. Offline/full gates pass at 418/0 with analyzers 0/0 and a one-executable package self-check. R6(b) dependency/SBOM inventory, R6(c) PSScriptAnalyzer, and R6(d) backlog review remain. |
