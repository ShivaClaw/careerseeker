...
| R6 | IN PROGRESS | R6(a) is complete on PR #25. R6(b) is BLOCKED on open PR #26: the local nine-package SPDX snapshot, advisory evidence, and offline/full gates are green (418/0; analyzers 0/0), but two CI attempts built 0/0 then found cross-host SPDX byte drift under PowerShell 7. See `docs/BETA-BLOCKED.md` and Q07; do not merge until both CI events pass. R6(c) PSScriptAnalyzer and R6(d) backlog review remain. |
