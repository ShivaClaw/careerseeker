...
- `scripts/Package-AlphaRelease.ps1` builds a trusted-tester ZIP with the published executable, native runtime
  dependencies, double-click setup/profile/provider/Gmail/demo/scout/selected-job/live/evidence/verify/dashboard launchers, workspace initializer, dashboard/helper
...
- Trusted-tester release ZIP now includes double-click setup, profile import, provider connect, Gmail connect, demo cycle, Scout board ingest, selected-job drafting, live alpha cycle, evidence export, package verification, and dashboard launchers, each
