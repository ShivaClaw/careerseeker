...
- **The declination, reason unchanged. This is the hundred-and-twelfth.** I resolved it from
  ...
  *"Decided 2026-08-07 (gate PQ-A6-1, default-proceed)"*; and the pin's `docs/sync-vectors/v1/`
  ...
  (**PQ-A2-3**). The prompt's one runnable ask ran **in this session**, twice — as `run-zero.sh` §2
  and directly in a clean worktree at the pin: `node docs/sync-vectors/generate.mjs --check` →
  **`OK: 29 vector files match the generator.`**, exit 0. **The slice is not merely built, it is
  SUBMITTED: draft PR #32, open 24 days, plus #37.** Rebuilding it would author another divergent
