...
- **The short-line rule, measured rather than asserted.** This firing's note was **measured at
  401** characters before insertion — **shorter than run 196's 409**, which is the rule's actual
  test, rather than asserted compliant. Nothing verified was dropped: the re-derivation above is
  cited to its commands.

- **Predecessor CI read per C-106-8:** run 196's tip `9ca4b3a` is android CI run **363**
  (id `34505666028`), `run_attempt` 1, conclusion **success**, completed
  2026-09-10T17:06:41Z. Per **C-117-4** a green re-verification of an
...
- **FOURTEENTH B-18 MESSAGE WITHHELD, by about four hours; the next firing is the one that meets the arm.** Run
  168 sent the **thirteenth** at **2026-09-06T01:00:03Z**. The predicate is a positive state
  trigger, or **five calendar days** — days, not runs. This run measured **2026-09-10T20:59:30Z**:
  elapsed **4d 19h 59m**, measured not incremented, so the arm falls **on or after
  2026-09-11T01:00:03Z** — about **four hours** out, which on this four-hourly cadence is
  **the 01:00Z firing, the very next run.** The **ESCALATION LEDGER** in the
  ...
  repo events, and a fourteenth four hours early would carry the same words to the same
