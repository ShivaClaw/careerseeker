...
- Keep offline harnesses (`Slice`, `EngineHarness`, `ResearcherHarness`,
  ...
  `DispatcherNoSendHarness`, `LifecycleHarness`, `RendererHarness`,
  `SyncHarness`) green before merging — `scripts/Verify-Alpha.ps1` runs these
  by default.
- `SyncHarness` reads `docs/sync-vectors/v1/`, which the Android repo's
  `:core` tests also consume. Those vectors are generated by
  `docs/sync-vectors/generate.mjs`; if you change them, regenerate rather than
  hand-editing (`node docs/sync-vectors/generate.mjs`, and `--check` to prove
  no drift). Changing a vector changes a published wire format that a second
  codebase depends on — treat it like the doc/verifier trap above, one unit
  that moves together. `docs/Sync-Protocol.md` is normative.
